...
function GenerateImages() {
  ...
  return (
   ...
   <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
    {imageStyle.map((item)=>(
      <span onClick={()=>setSelectedStyle(item)}  className={`text-xs px-7 items-center py-1 border rounded-full cursor-pointer ${
    selectedStyle === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'
  }`} key={item}>{item}</span>
    ))}
   </div>
   <div className='my-6 flex items-center gap-2'>
   <label htmlFor="make-public" className='relative cursor-pointer'>
   <input
    id="make-public"
    type='checkbox'
